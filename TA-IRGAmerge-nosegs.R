...
dsctemp <-data.frame(temp,time,dsc,mass)          #collect just the raw proteus data without header
...
proteus.new<-data.frame(temp,time,dsc,mass,co2.new,h2o.new)                #place all data in new dataframe
names(proteus.new)<-c("temp","time","dsc","mass","IRGAco2","IRGAh2o")
